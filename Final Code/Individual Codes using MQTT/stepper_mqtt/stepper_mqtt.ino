...
// DC MOTOR
const int ENA = D7;           // Enable pin for DC motor
const int IN1 = D1;           // IN3 pin for DC motor
const int IN2 = D2;           // IN4 pin for DC motor
...
long encoder_counts_per_rev = 2243;
int wheel_rev = 2;
...
void move_servo_motor(int SERVO_motion_type){
  ...
  if (SERVO_motion_type == 5){
    Serial.println("Servo Left command engaged....");
    servo_motor.write(180);       // LEFT
  }
  if (SERVO_motion_type == 6){    // RIGHT
    Serial.println("Servo right command engaged....");
    servo_motor.write(20);
  }
  if (SERVO_motion_type == 7){    // RESET
    Serial.println("Resetting the servo motor");
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (strcmp(message.c_str(), "startA")==0){
    Serial.println("Starting the autonomous system mode");
    stepper_motor.move(360 / 1.8 * 2);   // it should be * 2 if we have to move it 2 cm. Also assuming that CLOCKWISE DIRECTION IS THE ENGAGEMENT
    while (stepper_motor.distanceToGo() != 0){
      stepper_motor.run();
    }
  }

  if (strcmp(message.c_str(), "stopA") == 0){
    Serial.println("Stopping the autonomous system mode");
    stepper_motor.move(- 360 / 1.8 * 6);   // it should be * 2 if we have to move it 2 cm. Also assuming that CLOCKWISE DIRECTION IS THE ENGAGEMENT
    while (stepper_motor.distanceToGo() != 0){
      stepper_motor.run();
    }
  }

  if (strcmp(message.c_str(), "left")==0){
  ...
  if (strcmp(message.c_str(), "right")==0){
  ...
  // if (strcmp(message.c_str(), "straight")==0){
  //   Serial.println("First straight!!!");
  //   move_DC_motor(DC_STRAIGHT);
  // }
