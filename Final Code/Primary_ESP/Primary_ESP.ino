...
const int encoder_subtraction_value = 750;     // as i just want to move the walker by 50 cms
...
void LEFT_MOTION(){
  // move the servo motors first and then the DC for 30 cms ion
}
...
void move_DC_motor(int DC_motion_type){
  ...
  // CHARGING DOCK
  if (DC_motion_type == DC_CD){

    //rotate the DC motors clockwise; move 150 cms forward to the charging dock
    ...
    long US_next_target = 75;      // to check the ultrasonic sensors after every 1 cm
    long IR_next_target = 3750;    // to check the IR sensor after every 50 cms
    ...
    while (abs_encoderValue < target_encoder_counts){     
      ...
      if (abs_encoderValue >= US_next_target){      // to use encoder values i.e. to scan ultrasonic sensors on the amount of distance moved instead of time
        US_next_target += 75;
        if (!check_ultrasonic_sensors()){
          stop_DC_motor();
        }
      ...
      if (abs_encoderValue >= IR_next_target){
        stop_DC_motor();
        IR_next_target += 3750
        ...
        while (true){
          if (mySerial.availble()){
            ...
            if (receivedChar == 'N'){
              // that means i need to go straight
            }
            if (receivedChar == 'S'){
